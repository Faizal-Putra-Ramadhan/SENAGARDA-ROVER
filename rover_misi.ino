...
const int totalMisi = 4;
unsigned long durasiMajuMisi[totalMisi] = {
  3000, // A ke B = maju 2 detik
  5000, // B ke C = maju 3 detik
  3000, // C ke D = maju 2.5 detik
  5000  // D ke A = maju 4 detik
};
int misiSaatIni = 0;
...
void updateBelokKiri() {
  ...
  if (abs(errTurn) <= turnTolerance) {
    ...
    // Setelah selesai belok, pindah ke misi berikutnya
    misiSaatIni++;

    if (misiSaatIni >= totalMisi) {
      misiSaatIni = 0; // kembali ke misi 1 / A ke B
    }
    ...
    Serial.print("Lanjut ke MISI ");
    Serial.println(misiSaatIni + 1);

    Serial.print("Durasi maju misi berikutnya: ");
    Serial.print(durasiMajuMisi[misiSaatIni] / 1000.0);
...
void updateMajuPID() {
  ...
  // Ambil durasi maju sesuai misi saat ini
  unsigned long durasiMajuSekarang = durasiMajuMisi[misiSaatIni];

  // Kalau sudah maju sesuai durasi misi saat ini, mulai belok kiri
  if (millis() - modeStartTime >= durasiMajuSekarang) {
    motorStop();
    delay(300);

    Serial.println("================================");
    Serial.print("MISI ");
    Serial.print(misiSaatIni + 1);
    Serial.println(" SELESAI MAJU");
    Serial.println("Mulai belok kiri...");
    Serial.println("================================");
...
void setup() {
  ...
  Serial.println(setPoint);
  Serial.println("Daftar durasi maju setiap misi:");
  for (int i = 0; i < totalMisi; i++) {
    Serial.print("Misi ");
    Serial.print(i + 1);
    Serial.print(": ");
    Serial.print(durasiMajuMisi[i] / 1000.0);
    Serial.println(" detik");
  }
  Serial.println("================================");
}
